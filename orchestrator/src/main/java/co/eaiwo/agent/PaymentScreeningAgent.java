...
@Service
public class PaymentScreeningAgent {

    private final LlmService llm;
    private final ToolExecutorService executor;

    public PaymentScreeningAgent(LlmService llm,
    ...
    public String handle(String task) {
        ...
        history.add(new ChatMessage("system",
                "You are a specialist Payment Screening Agent for a UK bank. " +
                        "You screen payments for sanctions risk and assess payment risk. " +
                        "Always use sanctionsCheck to screen beneficiary names. " +
                        "Always use checkPaymentRisk to retrieve payment records and risk scores. " +
                        "Flag any CRITICAL or HIGH risk payments immediately. " +
                        "Give a clear assessment covering: sanctions status, payment risk level, and recommended action.",
                null, null));
        history.add(llm.userMsg(task));

        List<ToolDef> tools = llm.paymentTools();

        for (int turn = 0; turn < 5; turn++) {
            ChatCompletionResponse resp = llm.callWithTools(history, tools);
            var msg = resp.choices().get(0).message();

            System.out.println("[PaymentAgent] TURN " + turn
            ...
            if (msg.tool_calls() == null || msg.tool_calls().isEmpty()) {
                ...
                history.add(new ChatMessage("user",
                        "Summarise the payment screening findings clearly.", null, null));
                continue;
            }
            ...
            for (ToolCall call : msg.tool_calls()) {
                System.out.println("[PaymentAgent] TOOL: " + call.function().name());
        ...
        return "Payment screening could not be completed.";
    }
}